    ...
    fun encodeToSilk(sourcePath: String, destPath: String, start: Long, callback: ((Boolean) -> Unit)? = null) {
        val futureTask = object : FutureTask<Boolean>(Callable {
            ...
            if (!success) {
                ...
                if (success) {
                    try {
                        LogUtil.e(SilkHelper.javaClass.name, "silk初始化开始")
                        var successInt = MediaRecorder.silkEncInit()
                        LogUtil.e(SilkHelper.javaClass.name, "silk初始化： $successInt")
                        successInt = MediaRecorder.setVoiceSilkControl()
                        LogUtil.e(SilkHelper.javaClass.name, "silk设置压缩率： $successInt")
                        ...
                        LogUtil.e(SilkHelper.javaClass.name, "silk文件原大小： ${sourceBytes.size}")
                        ...
                        LogUtil.e(SilkHelper.javaClass.name, "silk编码结果： $successInt")
                        success = successInt == 0
                        File("$sourcePath.pcm").deleteOnExit()
                        LogUtil.e(SilkHelper.javaClass.name, "silk删除pcm文件： ")
                        MediaRecorder.silkEncUnInit()
                    } catch (t: Throwable) {
                        success = false
                        LogUtil.e(SilkHelper::class.java.name, "Silk Encode 出错 ${t.localizedMessage}")
                    }
                }
            }
